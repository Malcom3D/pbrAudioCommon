...
def _load_particle(particle_cfg: Any) -> Dict[int, Dict[str, Dict]]:
    """Load all particles poses sequence for a particles object."""
    data_path = particle_cfg.obj_path
...
def _generate_lib(header: str, generator: str, output_name: str, n_modes: int, n_vertices: int, t60_str: str, gain_waveform: str, freq_str: str, min_freq: float = None, max_freq: float = None, proxy_type: int = None, proxy_name: str = None, young_modulus: float = None, poisson_ratio: float = None, density: float = None, damping: float = None) -> str:

    material = ""
    if young_modulus is not None:
        material += f"E={young_modulus}, "
    if poisson_ratio is not None:
        material += f"ν={poisson_ratio}, "
    if density is not None:
        material += f"ρ={density}, "
    if damping is not None:
        material += f"ζ={damping}, "
        if min_freq is not None and max_freq is not None:
            alpha, beta = _compute_rayleigh_damping(min_freq, max_freq, damping)
            material += f"α={alpha}, β={beta}"

    proxy_str = ""
    if proxy_type is not None:
        proxy_str += f"type {proxy_type} "
    if proxy_name is not None:
        proxy_str += f"({proxy_name})"
...
// Mode frequencies (Hz)
modeFreqsUnscaled = ba.take(n+1, ({freq_str}));
...
def _compute_rayleigh_damping(f1: float, f2: float, xi1: float, xi2: float = None) -> Tuple[float, float]:
    ...
    f2 : float
        Second frequency (Hz)
    xi1 : float
        Damping ratio at f1 (dimensionless, e.g., 0.05 for 5%)
    xi2 : float
