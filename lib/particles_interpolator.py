...
from ..lib.functions import _load_particle
from ..lib.functions import debug_print, set_debug, set_debug_prefix
...
class ParticlesInterpolator:
    ...
    def _setup_numba_kernels(self):
        ...
        try:
            ...
            def _interpolate_quaternions_numba(q0, q1, qU, tU, t):
                ...
                For each element i:
                  - If t <= tU[i]: slerp between q0[i] (at time 0) and qU[i] (at time tU[i])
                  - If t >= tU[i]: slerp between qU[i] (at time tU[i]) and q1[i] (at time 1)
                """
                if qU.shape[0] > 0:
                    ...
                    for i in nb.prange(q0.shape[0]):
                        ...
                        # Select the pair of quaternions and the local interpolation parameter
                        if t <= tUi:
                            # Interpolate between q0 (t=0) and and qU (t=tUi)
                            a0 = q0[i, 0]; a1 = q0[i, 1]; a2 = q0[i, 2]; a3 = q0[i, 3]
                            b0 = qU[i, 0]; b1 = qU[i, 1]; b2 = qU[i, 2]; b3 = qU[i, 3]
                            # Local parameter s in [0, 1] within [0, tUi]
                            if tUi > 1e-12:
                                s = t / tUi
                            else:
                                s = 0.0
                        ...
                        if sin_theta_0 > 1e-6:
                            ...
                            # Handle near-zero angle case (use linear interpolation)
                            s0 = 1.0 - s
                            s1 = s

                        # Interpolate
                        result[i, 0] = s0 * a0 + s1 * b0
                        result[i, 1] = s0 * a1 + s1 * b1
                        result[i, 2] = s0 * a2 + s1 * b2
    ...
    def load_frame(self, frame: int, unsampled: bool = False) -> Optional[Dict[str, np.ndarray]]:
        ...
        if self._is_static or len(self._frames) == 1:
            ...
            file_path = Path(self._obj_path) / f"{self._particle_name}_{frame:05d}{interframe}.npz"
            if not file_path.exists():
                file_path = Path(self._obj_path) / f"{self._particle_name}_{frame}{interframe}.npz"
            if not file_path.exists():
                # Try finding the frame in the list
                npz_files = sorted(Path(self._obj_path).glob("*.npz"))
