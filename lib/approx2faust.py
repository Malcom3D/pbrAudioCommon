...
from ..lib.functions import _compute_rayleigh_damping, _generate_lib, _generate_empty_lib
...
class Approx2Faust:
    ...
    def to_faust_lib(self, modal_params: Dict[str, Any], output_name: str, min_freq: float, max_freq: float, t60_scale: float = 1) -> str:
        """
        Generate and save Faust .lib file content for modal synthesis
...
#        n_vertices = metadata.get('n_vertices', 0) if gains.shape[1] > 0 else 0
        ...
        if n_modes == 0 or n_vertices == 0:
            print(f'Warning: {output_name} own {n_vertices} vertices and {n_modes} modes. Generating fake lib file')
